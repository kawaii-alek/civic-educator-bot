...
const ChatInterface = ({ setIsSidebarOpen, language, setLanguage }: ChatInterfaceProps) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50 relative">
      ...
      <header className="px-6 md:px-10 py-6 md:py-8 bg-white border-b border-slate-200 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-serif font-bold text-slate-900">{t.title}</h1>
            <p className="text-xs md:text-sm text-slate-500 mt-0.5 md:mt-1">{t.subtitle}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex items-center">
            <select
              ...
              className="appearance-none pl-9 pr-8 py-2 text-xs font-semibold rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors border border-slate-200 outline-none cursor-pointer"
            >
              {LANGUAGES.map(lang => (
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-6 md:p-12 space-y-10 scrollbar-thin scrollbar-thumb-slate-200">
        <AnimatePresence initial={false}>
          {messages.map((msg, index) => (
            ...
            >
              <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center flex-shrink-0 border ${
                msg.role === 'bot' 
                  ? 'bg-amber-50 border-amber-200 text-amber-700' 
                  : 'bg-slate-900 border-slate-800 text-white'
              }`}>
                {msg.role === 'bot' ? <LandmarkIcon /> : <User size={18} />}
              </div>
              <div className={`p-4 md:p-6 rounded-2xl shadow-sm border max-w-[85%] md:max-w-[70%] ${
                msg.role === 'bot' 
                  ? 'bg-white border-slate-200 text-slate-800' 
                  : 'bg-slate-800 border-slate-700 text-white'
              }`}>
                {msg.role === 'bot' && <Quote size={16} className="text-amber-300 mb-2 opacity-30" />}
                <p className="leading-relaxed font-serif text-base md:text-lg">{msg.content}</p>
      ...
      {/* Input Area */}
      <div className="p-6 md:p-12 pt-0 bg-slate-50">
        <div className="bg-white p-2 border border-slate-200 rounded-xl flex items-center gap-2 md:gap-4 shadow-lg ring-4 ring-slate-100">
          <input
            type="text"
            value={input}
            ...
            className="flex-1 bg-transparent border-none outline-none text-slate-800 placeholder:text-slate-400 px-4 md:px-6 font-serif text-base md:text-lg"
            disabled={isLoading}
          />
          <button
            onClick={handleSend}
            disabled={isLoading || !input.trim()}
            className="px-4 md:px-6 h-12 md:h-14 rounded-lg bg-amber-600 text-white flex items-center gap-2 md:gap-3 font-bold hover:bg-amber-700 transition-colors disabled:opacity-50 text-sm md:text-base whitespace-nowrap"
          >
